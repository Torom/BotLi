...
def validate_time_limits(initial: int,
                         increment: int,
                         min_initial: int,
                         max_initial: int,
                         min_increment: int,
                         max_increment: int) -> tuple[bool,
                                                      str | None]:
...
def validate_config_section(
        config: dict,
        section_name: str,
        required_fields: list[list]) -> None:
    for field_info in required_fields:
        field_name, field_type, error_msg = field_info
        if field_name not in config:
            raise RuntimeError(
                f"Your config does not have required `{section_name}` subsection `{field_name}`.")
