...
class Lichess_Game:
    ...
    async def make_move(self) -> Lichess_Move:
        for move_source in self.move_sources:
            if move_response := await move_source():
                ...
                return Lichess_Move(
                    move_response.move.uci(), self._offer_draw(
                        move_response.trusted_eval, move_response.is_draw), self._resign(
                        move_response.trusted_eval, move_response.is_lost), )
    ...
    def _resign(self, is_trusted: bool = True,
    ...
    async def _make_book_move(self) -> Move_Response | None:
        ...
        for name, book_reader in self.book_settings.readers.items():
            ...
            for entry in entries:
                if self.book_settings.allow_repetitions or not self._is_repetition(
                        entry.move):
                    break
            else:
                continue
    ...
    def _get_book_settings(self) -> Book_Settings:
        ...
        return Book_Settings(books_config.selection,
                             ...
                             {name: chess.polyglot.open_reader(path) for name,
                              path in books_config.names.items()},
                             )
    ...
    async def _make_opening_explorer_move(self) -> Move_Response | None:
        ...
        if not self.config.online_moves.opening_explorer.allow_repetitions and self._is_repetition(
                move):
            return
    ...
    async def _make_cloud_move(self) -> Move_Response | None:
        ...
        if not self.config.online_moves.lichess_cloud.allow_repetitions and self._is_repetition(
                pv[0]):
            return
        ...
        message = f"Cloud:   {
            self._format_move(
                pv[0]):14} {
            self._format_score(score)}     Depth: {
                response['depth']}"
        return Move_Response(
            pv[0],
            message,
            pv=pv,
            trusted_eval=self.config.online_moves.lichess_cloud.trust_eval)
    ...
    async def _make_chessdb_move(self) -> Move_Response | None:
        ...
        if not self.config.online_moves.chessdb.allow_repetitions and self._is_repetition(
                pv[0]):
            return
        ...
        return Move_Response(
            ...
            message,
            pv=pv,
            trusted_eval=self.config.online_moves.chessdb.trust_eval)
    ...
    def _get_move_sources(
        ...
        sources.extend(
            [source.method for source in opening_sources if source.is_available])

        return sources

    def _get_endgame_sources(
            self) -> list[Callable[[], Awaitable[Move_Response | None]]]:
        sources: list[Callable[[], Awaitable[Move_Response | None]]] = []
        ...
        if self.config.online_moves.online_egtb.enabled and self.board.uci_variant in [
                "chess", "antichess", "atomic"]:
        ...
        if explorer_config.enabled:
            sources.append(
                Move_Source(
                    ...
                    conditions=[
                        self._check_book_condition(
                            explorer_config.only_without_book),
                        self._check_variant_condition(
                            explorer_config.use_for_variants),
                        self._check_variant_condition(
                            explorer_config.player != "masters"),
                    ],
                ))
        ...
        if cloud_config.enabled:
            sources.append(
                Move_Source(
                    ...
                    conditions=[
                        self._check_book_condition(
                            cloud_config.only_without_book),
                        self._check_variant_condition(
                            cloud_config.use_for_variants),
                    ],
                ))
        ...
        if chessdb_config.enabled:
            sources.append(
                Move_Source(
                    ...
                    conditions=[
                        self._check_book_condition(
                            chessdb_config.only_without_book),
