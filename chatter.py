...
COMMANDS = {
    ...
    "depth": "Shows the search depth from the latest engine calculation.",
    ...
    "limits": "Shows the current engine calculation limits (time, depth, nodes).",
...
class Chatter:
    ...
    async def _handle_command(self, chat_message: Chat_Message, takeback_count: int, max_takebacks: int) -> None:
        match chat_message.text[1:].lower():
            ...
            case "depth":
                await self._send_depth_message(chat_message.room)
            case "draw":
            ...
            case "limits":
                await self._send_limits_message(chat_message.room)
            case "motor":
    ...
    async def _send_limits_message(self, room: str) -> None:
        limit_config = self.lichess_game.engine.limit_config
        limits = []
        if limit_config.time is not None:
            limits.append(f"Time: {limit_config.time}s")
        if limit_config.depth is not None:
            limits.append(f"Depth: {limit_config.depth}")
        if limit_config.nodes is not None:
            if limit_config.nodes >= 1_000_000:
                limits.append(f"Nodes: {limit_config.nodes / 1_000_000:.1f}M")
            elif limit_config.nodes >= 1_000:
                limits.append(f"Nodes: {limit_config.nodes / 1_000:.1f}K")
            else:
                limits.append(f"Nodes: {limit_config.nodes}")
        message = f"Limits: {', '.join(limits)}" if limits else "No engine limits configured"
    ...
    async def _send_depth_message(self, room: str) -> None:
        ...
        if depth_match:
            ...
            message = "No depth information available yet"
        await self.api.send_chat_message(self.game_info.id_, room, message)
            ...
            with open("/proc/cpuinfo", encoding="utf-8") as cpuinfo:
                while line := cpuinfo.readline():
                    if line.startswith("model name"):
                        cpu = line.split(": ")[1].replace("(R)", "").replace("(TM)", "")
                        if len(cpu.split()) > 1:
                            return cpu
    ...
    def _append_pv(self, initial_message: str = "") -> str:
        ...
        if initial_message:
            initial_message += " "
        board = self.lichess_game.board.copy(stack=not self.lichess_game.is_our_turn)
        if self.lichess_game.is_our_turn:
            board.pop()
        initial_message += "PV:" if board.turn else f"PV: {board.fullmove_number}..."
